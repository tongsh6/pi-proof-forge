...
- [x] A1 新增 `tools/domain/models.py`
...
- [x] A2 新增 `tools/domain/value_objects.py`
...
- 新增 ReviewCandidate（job_lead_id / company / position / matching_score / evaluation_score / round_index / resume_version）和 ReviewDecision（job_lead_id / action: approve|reject|skip|skip_all / decided_by / decided_at / note）
  - 所有值对象不可变、可比较、可序列化
- [x] A3 新增 `tools/domain/protocols.py`
...
- [x] A11 新增 `tools/config/loader.py` + `tools/config/validator.py`
...
- [x] B1 新增 `tools/engines/registry.py`
  - 实现通用 `EngineRegistry[T]`
  - 支持 register/create/list 三个最小能力
  - 未注册策略必须在启动或构建阶段失败，禁止隐式回退

### B-Evidence（证据引擎）
- [x] B2 新增 `tools/engines/evidence/rule_extractor.py`
  - 迁移 `extract_evidence.py` 规则逻辑
  - 仅处理领域对象，不依赖文件路径或 CLI
- [x] B3 新增 `tools/engines/evidence/llm_extractor.py`
  - 注入 `LLMClient`，不在内部读取环境变量
  - 通过 registry 注册为 `evidence_mode=llm`
- [x] B4 新增 `tools/engines/evidence/validator.py`
  - 强制校验 `results/artifacts`，违反抛 `EvidenceValidationError`
  - 验证 `raw_source` 引用存在且可追溯
- [x] B5 新增 `tools/engines/evidence/store.py`
  - 证据卡读取、写入、按 ID 检索
  - 路径通过 `PathConfig` 注入，不硬编码

### B-Matching（匹配评分引擎）
- [x] B6 新增 `tools/engines/matching/rule_scorer.py`
  - `RuleMatchingEngine`：K/D/S/Q/E/R 六维度规则评分
  - 迁移 `build_rule_report` 核心逻辑
- [x] B7 新增 `tools/engines/matching/llm_matcher.py`
  - `LLMMatchingEngine`：注入 `LLMClient` + prompt 路径
  - 构造器注入，不含 `if use_llm` 分支
- [x] B8 新增 `tools/engines/matching/report_builder.py`
  - 构造 `MatchingReport`
  - 强制生成 `gap_tasks`，不得静默忽略缺口

### B-Generation（简历生成引擎）
- [x] B9 新增 `tools/engines/generation/template_assembler.py`
  - 迁移 `build_template_resume` 核心逻辑
  - 无对应 EvidenceCard 的内容禁止生成，违反抛 `FabricationGuardError`
  - 必须先写红灯测试，验证无证据内容会触发守卫
- [x] B10 新增 `tools/engines/generation/llm_rewriter.py`
  - 受控改写语气，不引入新事实
  - 必须把 `jd_context` 和 `company_context` 注入 prompt
- [x] B11 新增 `tools/engines/generation/exporter.py`
  - Markdown 导出（基准实现）
  - PDF 导出接口（占位，后续实现）

### B-Evaluation（质量评测引擎）
- [x] B12 新增 `tools/engines/evaluation/rule_evaluator.py`
  - `RuleEvaluationEngine`：coverage/quant/clarity/length/citation 五维度
  - 迁移 `evaluate_rule` 核心逻辑
- [x] B13 新增 `tools/engines/evaluation/llm_evaluator.py`
  - 先执行规则评测，再用 LLM 补充解释层
  - 规则分数不可被 LLM 修改
- [x] B14 新增 `tools/engines/evaluation/scorecard_builder.py`
  - 构造 `Scorecard`，每项质量分必须可追溯到具体检查点

### B-Discovery（候选发现引擎）
- [x] B15 新增 `tools/engines/discovery/discovery_engine.py`
...
### B-Registry Wiring
- [x] B16 完成四大引擎 + discovery 的 registry 注册
...
- [x] C4a 新增 `tools/orchestration/review_stage.py`
...
- [x] D5 新增 `tools/infra/persistence/file_run_store.py`
  - 输出 `outputs/agent_runs/<run_id>/run_log.json`
  - 事件追加写入，不回写历史
  - 轮次快照：`rounds/<n>/`
- [x] D6 实现 `RunStore.load_events(run_id)`
  - 支持状态回放、断点续跑、审计追溯
...
- [x] E5 更新 `tools/README.md` 与根 `README.md`
  - 新增 v2 架构说明与 `run_agent` 使用示例
  - 说明 `domain/`、`infra/`、`engines/`、`orchestration/` 的职责
- [x] E6 AIEF L3 检查通过（`python3 tools/check_aief_l3.py --root . --base-dir AIEF`）
- [x] E7 新增 CI/静态校验任务
...
- [x] 单命令可触发完整 Agent 循环（`python3 tools/run_agent.py --policy ...`）
- [x] N-pass 门禁生效且有日志可追溯（strict/simulate 行为符合预期）
- [x] 至少一个通道可在 dry-run / simulate 条件下完成端到端投递链路并产出可追溯 artifact
- [x] 达到 `max_rounds` 或 `max_deliveries` 时可安全停止
- [x] 旧入口脚本保持兼容（无破坏性参数变化）
- [x] 4 处 LLM 客户端重复已消除，统一使用 `infra/llm/client.py`，并由静态检查脚本验证
- [x] 3 处 YAML 解析重复已消除，统一使用 `infra/persistence/yaml_io.py`，并由静态检查脚本验证
- [x] 所有业务层 subprocess 串联已消除，改为纯 Python API 调用，并由静态检查脚本验证
- [x] 所有业务引擎内不再出现 `if use_llm` 分支，并由静态检查脚本验证
- [x] 生成产物中的结论均可追溯到具体 EvidenceCard，守卫测试通过
- [x] Domain 单测 + 引擎单测全部通过
- [x] Result 类型已用于可恢复错误（gate/channel/discovery）
- [x] RunState 支持事件回放恢复
- [x] 企业例外清单已在 discovery 主过滤 + gate 兜底两层生效
- [x] auto/manual 双模式可通过 delivery_mode 配置切换，且 auto 下 REVIEW 为 pass-through
- [x] manual 模式下 ReviewStage 可正确暂停并等待用户审批
- [x] AIEF L3 检查通过
