...
const ARTIFACT_DIR = join(UI_DIR, "test-results", "evidence-native");
...
function parseArgs(argv) {
  const args = {
    timeoutMs: Number(process.env.EVIDENCE_NATIVE_VERIFY_TIMEOUT_MS || "90000"),
    keepArtifacts: false,
  };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--help" || arg === "-h") {
      console.log(
        "Usage: node ui/scripts/verify_evidence_native.mjs [--timeout-ms <ms>] [--keep-artifacts]"
      );
      process.exit(0);
    }
    if (arg === "--timeout-ms") {
...
function validateReadyEvent(event) {
  if (event.event !== "evidence.load.ready") return false;
  for (const key of ["card_count", "artifact_count"]) {
...
async function waitForReadyEvent(args, child) {
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(
        `Tauri dev exited before Evidence loaded, exit code ${child.exitCode}`
      );
    }
  ...
  throw new Error("Timed out waiting for Evidence native ready event");
}
