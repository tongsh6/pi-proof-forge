...
function parseArgs(argv) {
  const args = {
    timeoutMs: Number(process.env.SUBMISSIONS_NATIVE_VERIFY_TIMEOUT_MS || "90000"),
    keepArtifacts: false,
  };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--help" || arg === "-h") {
      console.log(
        "Usage: node ui/scripts/verify_submissions_native.mjs [--timeout-ms <ms>] [--keep-artifacts]"
      );
      process.exit(0);
    }
    if (arg === "--timeout-ms") {
...
function validateReadyEvent(event) {
  if (event.event !== "submissions.load.ready") return false;
  for (const key of [
    "submission_count",
    "delivered_count",
    "failed_count",
...
async function waitForReadyEvent(args, child) {
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(
        `Tauri dev exited before Submissions loaded, exit code ${child.exitCode}`
      );
    }
  ...
  throw new Error("Timed out waiting for Submissions native ready event");
}
