...
const ARTIFACT_DIR = join(UI_DIR, "test-results", "policy-native");
...
function parseArgs(argv) {
  const args = {
    timeoutMs: Number(process.env.POLICY_NATIVE_VERIFY_TIMEOUT_MS || "90000"),
    keepArtifacts: false,
  };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--help" || arg === "-h") {
      console.log(
        "Usage: node ui/scripts/verify_policy_native.mjs [--timeout-ms <ms>] [--keep-artifacts]"
      );
      process.exit(0);
    }
    if (arg === "--timeout-ms") {
...
function validateReadyEvent(event) {
  if (event.event !== "policy.load.ready") return false;
  if (!["auto", "manual"].includes(event.delivery_mode)) return false;
  if (typeof event.batch_review !== "boolean") return false;
  if (event.delivery_mode === "auto" && event.batch_review) return false;
  if (typeof event.company_rule_count !== "number") return false;
  if (typeof event.legal_entity_rule_count !== "number") return false;
  return true;
}
...
async function waitForReadyEvent(args, child) {
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(
        `Tauri dev exited before Policy loaded, exit code ${child.exitCode}`
      );
    }
  ...
  throw new Error("Timed out waiting for Policy native ready event");
}
  ...
  try {
    ...
    console.log(
      JSON.stringify({
        ...
        batch_review: event.batch_review,
        company_rule_count: event.company_rule_count,
        legal_entity_rule_count: event.legal_entity_rule_count,
        log: logPath,
