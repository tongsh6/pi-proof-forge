...
> 2026-05-20 补充：用户旅程验收 M2 报告生成器已完成。新增 `tools/acceptance/journey_report.py`，可从 `JourneyContract` 生成稳定 JSON/Markdown 报告，字段覆盖 run_id、contract_version、summary、case/rule/level/stage/phase/status/evidence/message；同时提供 artifact check helper，用于把缺失产物归因到具体 phase（例如 evaluation scorecard）。报告默认写入 `outputs/acceptance/<run_id>/journey-report.json` 与 `.md`，并可通过 `python3 -m tools.acceptance.journey_report --run-id <id>` 直接生成 baseline 报告。验证：`python3 -m pytest tests/acceptance/test_journey_contract.py tests/acceptance/test_journey_report.py -q` → 9 passed；`python3 -m tools.acceptance.journey_report --run-id journey_report_verify_20260520` → pass；`python3 tools/check_v2_constraints.py --root .` → PASS。
> 2026-05-20 补充：用户旅程验收 M1 Case 1 第一段已接线。`settings.update` 支持 `llm_config`，可保存 LM Studio provider/base_url/model/timeout/temperature 与 API key configured 状态；`settings.get` 继续只返回 masked secret 状态，不返回明文。新增 `settings.checkLlmConnection` 只读 RPC，通过 infra `LLMClient.list_models()` 检查 `/models`，不可达、空模型或未配置 base_url 时返回结构化 `BLOCKED_LOCAL_PROVIDER`，不抛 generic crash。新增 `tests/acceptance/fixtures/scenarios/first_launch_configure_lm_studio/lm_studio_config.json` 与 `tests/acceptance/test_scenario_first_launch_configure_lm_studio.py`，可生成 blocked journey report。验证：`python3 -m pytest tests/unit/infra/test_llm_client.py tests/unit/sidecar/test_settings_handler.py tests/unit/sidecar/test_server.py tests/acceptance/test_scenario_first_launch_configure_lm_studio.py tests/acceptance/test_journey_report.py -q` → 56 passed；`pnpm --dir ui build` → pass。
> 2026-05-21 补充：用户旅程验收 M1.4 已完成。Quick Run 与 Agent Run 页面现在复用既有 `settings.get` 读取掩码后的 `llm_config`，展示 provider/base_url/model/API key configured 状态，不新增 RPC 方法、不读取或展示明文 secret；Case 1 acceptance report 将“连接检查 blocked”和“运行页可读 provider summary”拆成独立规则，避免本地 LM Studio 未启动时误判运行页不可见。验证：`python3 -m pytest tests/unit/gui/test_quick_run_page_contract.py tests/unit/gui/test_agent_run_page_contract.py tests/acceptance/test_scenario_first_launch_configure_lm_studio.py tests/acceptance/test_journey_contract.py -q` → 12 passed；`python3 -m pytest tests/ -q` → 421 passed；`pnpm --dir ui build` → pass；`python3 tools/check_v2_constraints.py --root .` → PASS；`python3 tools/check_aief_l3.py --root . --base-dir AIEF` → PASS；`pnpm --dir ui run e2e:agent-run` → pass；`pnpm --dir ui run e2e:quick-run` → pass（`qr_20260520161828157900`）。
> 2026-05-21 补充：用户旅程验收 M5 runner 已完成。新增 `scripts/acceptance/run-acceptance.sh` 与 `tools/acceptance/acceptance_runner.py`，默认执行已实现的 L1 场景验收，支持 `--level L1|L2|L3` 和 `--all`；L2/L3 当前保持 gated `not_started`，不会被 L1 误标为通过。报告写入 `outputs/acceptance/<run_id>/acceptance-report.json` / `.md`，并同时生成 `journey-report.json` / `.md`。验证：`python3 -m pytest tests/acceptance/test_acceptance_runner.py tests/acceptance/test_acceptance_runner_script.py -q` → 9 passed；`bash scripts/acceptance/run-acceptance.sh --run-id acceptance_verify_20260521 --level L1` → pass；`bash scripts/acceptance/run-acceptance.sh --run-id acceptance_l2_gated_20260521 --level L2` → not_started（退出 0）。
> 2026-05-21 补充：用户旅程验收 Case 2 `setup_profile_and_material_library` L1 已完成。新增 sidecar 材料库 `material.upload/list/readiness` 与 `evidence.listMaterialSources`，支持 `.md/.txt` 工作材料保存、文本预览、资料库 ready/incomplete 缺项反馈，并扩展 `resume.upload/getPreview` 支持 Markdown/Text 简历上传与预览；用户本地上传目录 `uploaded_resumes/`、`materials/`、`app-data/` 已加入 `.gitignore`。Acceptance runner 现在逐 L1 场景执行并按场景映射 journey rule，避免一个场景失败污染其他 L1 规则。验证：`bash scripts/acceptance/run-acceptance.sh --run-id acceptance_profile_materials_20260521 --level L1` → pass；`python3 -m pytest tests/ -q` → 437 passed；`pnpm --dir ui build` → pass；`python3 tools/check_v2_constraints.py --root .` → PASS；`python3 tools/check_aief_l3.py --root . --base-dir AIEF` → PASS。
> 2026-05-21 补充：System Settings 的 LLM 配置已从只读摘要推进为用户可见编辑闭环。页面现可编辑 provider/model/base_url/API key/timeout/temperature，调用既有 `settings.update` 保存并继续只回读 masked secret 状态；连接检查调用既有 `settings.checkLlmConnection`，显示 pass/blocked 结构化结果，不新增 sidecar 协议。System Settings native verifier 已扩展为等待真实 Tauri 页面写出 `system_settings.llm.form.ready`，确认 LLM 表单进入桌面壳。验证：`python3 -m pytest tests/unit/gui/test_system_settings_page_contract.py tests/unit/sidecar/test_settings_handler.py tests/acceptance/test_scenario_first_launch_configure_lm_studio.py -q` → 30 passed；`pnpm --dir ui build` → pass；`pnpm --dir ui run e2e:system-settings` → pass（`llm_form_ready=true`）；`python3 -m pytest tests/ -q` → 438 passed；`bash scripts/acceptance/run-acceptance.sh --run-id acceptance_llm_settings_gui_20260521 --level L1` → pass；v2 与 AIEF L3 均通过。
...
- **用户场景化自动验收 → scenario-first M-1、M0 case-aware journey contract、M2 report generator 与 M5 L1 runner 已完成；M1 Case 1 settings persistence、structured connection check、Quick Run / Agent Run provider summary 与 System Settings LLM 可编辑 GUI 已接线；Case 2 profile/material library L1 API 验收已接线；optional L3-local、M3 其他 GUI smoke 与 M4 external check-mode 尚未接线。**
